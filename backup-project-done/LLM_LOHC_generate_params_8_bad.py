...
INITIAL_SET_SIZE = 30        # Number of molecules taken from CSV
...
def old_is_valid_smiles(smiles):
...
invalid_smiles_tracker = {}  # Dictionary to store invalid SMILES count per iteration

def is_valid_smiles(smiles, iteration):
    """Checks if a SMILES string is valid and returns its canonical form."""
    global invalid_smiles_tracker  # Ensure we're updating the global tracker
    
    mol = Chem.MolFromSmiles(smiles)
    if mol is None:
        print(f"❌ Invalid SMILES detected in iteration {iteration}: {smiles}")  # Log invalid SMILES
        
        # Increment count for this iteration
        if iteration not in invalid_smiles_tracker:
            invalid_smiles_tracker[iteration] = 0
        invalid_smiles_tracker[iteration] += 1
        
        return None  # Return None for invalid structures
...
def old_5_iterative_generation(initial_set):
    """Generates LOHC SMILES iteratively until reaching TARGET_COUNT, ensuring uniqueness at this step."""
    iterative_set = set()
    final_results = []
...
def iterative_generation(initial_set):
    ...
    final_smiles_set = set()
    initial_smiles_set = set(initial_set)
    invalid_smiles_tracker = {}  # Track invalid SMILES per iteration

    for i in range(MAX_ITERATIONS):
        if len(iterative_set) >= TARGET_COUNT:
            break

        iteration_number = i + 1  # Track current iteration
        invalid_smiles_tracker[iteration_number] = 0  # Initialize count for this iteration

        start_time = time.time()  # Start iteration timer
        print(f"\n🔹 Iteration {iteration_number}/{MAX_ITERATIONS} - Current set size: {len(iterative_set)}.")

        new_smiles = generate_new_smiles(list(iterative_set) + list(initial_smiles_set))

        # Track invalid SMILES and validate structures
        validated_smiles = []
        for s in new_smiles:
            canonical_s = is_valid_smiles(s)
            if canonical_s:
                validated_smiles.append(canonical_s)
            else:
                invalid_smiles_tracker[iteration_number] += 1  # Count invalid SMILES

        new_smiles = list(set(validated_smiles))  # Remove duplicates after validation
        ...
        # Store results and track new SMILES
        for r in unique_smiles:
            final_smiles_set.add(r["SMILES"])
            iterative_set.add(r["SMILES"])
            final_results.append(r)
        ...
        # Log invalid SMILES count
        print(f"🔹 Total Invalid SMILES in iteration {iteration_number}: {invalid_smiles_tracker[iteration_number]}")
        print(f"⏳ Iteration {iteration_number} completed in {elapsed_time:.2f} seconds.")
    ...
    return final_results, invalid_smiles_tracker  # Return results & invalid SMILES log
